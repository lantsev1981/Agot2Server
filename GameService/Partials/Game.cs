...
namespace GameService
{
    public partial class Game
    {
        ...
        {
            get { return this.Vesteros.LastStep.GameInfo; }
        }

        /// <summary>
        /// Отсортированный список карточных колод вестероса и одичалых
        /// </summary>
        public List<VesterosDecks> VesterosDecks
        {
            get { return this.GameInfo.VesterosDecks.OrderBy(p => p.Sort).ToList(); }
        }
        ...
        /// </summary>
        public List<Step> LastHomeSteps
        {
            get
            {
                return this.HomeUsersSL.Select(p => p.LastStep)
        ...
        /// </summary>
        public List<Step> HomeSteps
        {
            get
            {
                return this.HomeUsersSL.SelectMany(p => p.Step)
                    .OrderBy(p => p.Id).ToList();
            }
        }
        ...
        public List<Step> AllSteps
        {
            get
            {
                return this.GameUser.SelectMany(p => p.Step)
                    .OrderBy(p => p.Id).ToList();
            }
        }
        ...
        /// </summary>
        public List<GameUser> HomeUsersSL
        {
            get
            {
                return this.GameUser.Where(p => !string.IsNullOrEmpty(p.HomeType))
                    .OrderBy(p => p.LastStep.GameUserInfo.ThroneInfluence).ToList();
            }
        }

        public List<GameUser> VotedUsers { get { return HomeUsersSL.Where(p => p.LastStep.Voting != null).ToList(); } }

        public WCFRateSettings RateSettings
        {
            get
            {
                return new WCFRateSettings()
                {
        ...
        public WCFGame ToWCFGame()
        {
            ...
            result.Settings = new WCFGameSettings()
            {
                GameType = this.Type,
            ...
            result.GameUser = new List<WCFGameUser>();
            foreach (var item in this.GameUser.ToList())
        ...
        private void AddAllContent()
        {
            ...
            var firstStep = LastHomeSteps.First();
            var lastStep = LastHomeSteps.Last();
            ...
            DbContext.Terrain.ToList().ForEach(p =>
            {
                ...
                {
                    var powerCounter = new PowerCounter()
                    {
                        Terrain = p.Name,
                        Terrain1 = p,
        ...
        {
            var knight = DbContext.UnitType.Single(p => p.Name == "Рыцарь");
            var board = DbContext.UnitType.Single(p => p.Name == "Корабль");

            var units = step.GameUserInfo.Unit.Where(p => p.Terrain == terrain.Name);
        ...
        public void NewTurn()
        {
            ...
            this.GameInfo.Turn++;
            newVesterosStep.NewMessage("dynamic_newTurn*" + this.GameInfo.Turn);
        ...
        public void NewBarbarian(bool isVictory, GameUser user)
        {
            Step newVesterosStep = this.Vesteros.LastStep.CopyStep("Default", true);
            if (isVictory)
                this.GameInfo.Barbarian = 0;
            else
            {
                if (this.GameInfo.Barbarian < 4)
                    this.GameInfo.Barbarian = 0;
                else
                    this.GameInfo.Barbarian -= 4;
            }
            ...
            switch (vesterosDecks.VesterosCardType1.Name)
            {
                ...
                case "Разведчик-оборотень":
                    ...
                    else
                    {
                        ...
                        foreach (var item in this.VotedUsers)
                        {
                    ...
                    else
                    {
                        ...
                        foreach (var item in this.VotedUsers)
                        {
                            ...
                            if (user == item)
                            {
                                ...
                                foreach (var card in notUsed.Where(p => p.Strength == maxStrength).ToList())
                                {
                                    if (notUsed.Count() > 1)
                                    {
                ...
                case "Убийцы_ворон":
                    ...
                    if (isVictory)
                    {
                        ...
                        if (availableKnight > 0 && footManCount > 0)
                        {
                            ...
                            newStep.NewMarch();
                            foreach (var unit in newStep.GameUserInfo.Unit.Where(p => p.UnitType == "Пеший_воин").ToList())
                            {
                                newStep.March.MarchUnit.Add(new MarchUnit()
                                {
                                    Id = Guid.NewGuid(),
                    ...
                    else
                    {
                        foreach (var item in this.VotedUsers)
                        {
                            ...
                            if (knightCount > 0)
                            {
                                ...
                                foreach (var unit in newStep.GameUserInfo.Unit.Where(p => p.UnitType == "Рыцарь").ToList())
                                {
                                    newStep.March.MarchUnit.Add(new MarchUnit()
                                    {
                                        Id = Guid.NewGuid(),
                ...
                case "Наступление_орды":
                    ...
                    if (isVictory)
                    {
                        ...
                        foreach (var terrain in newStep.GameUserInfo.GameUserTerrain.Select(p => p.Terrain1).Where(p => p.Strength != 0).ToList())
                        {
        ...
        public void ResetVesterosDesc(int decksNumber)
        {
            ...
            foreach (var item in vesterosDecks)
            {
                item.IsFull = false;
        ...
        public void VesterosAction(string vesterosActionType)
        {
            switch (vesterosActionType)
            {
                ...
                case "Сбор власти":
                    foreach (var user in this.HomeUsersSL)
                    {
                        Step newStep = user.LastStep.CopyStep("Default", true);

                        foreach (var terrain in newStep.GameUserInfo.GameUserTerrain.Select(p => p.Terrain1))
                        {
        ...
        public void NewVoting(string votingTarget, GameUser expUser = null)
        {
            ...
            foreach (var user in votingUser)
            {
        ...
        public void NewThink()
        {
            foreach (var user in this.HomeUsersSL)
            {
                Step step = user.LastStep.CopyStep("Замысел", true);

                //включаем меч и поднимаем юнитов
                step.GameUserInfo.IsBladeUse = false;
                foreach (var item in step.GameUserInfo.Unit.ToList())
        ...
        private void RemoveTerrainHolder(Terrain terrain, GameUser newHolder = null)
        {
            ...
            if (terrain.TerrainPort != null)
            {
                ...
                foreach (var ship in oldHolderStep.GameUserInfo.Unit.Where(p => p.Terrain1 == terrain.TerrainPort).ToList())
                {
                    if (newHolder != null)
                    {
        ...
        public void NewBattle(GameUser attackUser, Terrain attackTerrain, Terrain defenceTerrain)
        {
            Step newVesterosStep = this.Vesteros.LastStep.CopyStep("Default", true);
            ...
            battle.AttackTerrain = attackTerrain.Name;
            battle.DefenceTerrain = defenceTerrain.Name;
            battle.AttackUser = attackUser.Id;
            battle.DefenceUser = defenceUser.Id;

            battle.Step = newVesterosStep.Id;
            battle.Game = this.Id;
            battle.GameInfo = this.GameInfo;
        ...
        internal void TheEnd()
        {
            ...
            List<GameUser> users = this.LastHomeSteps.OrderByDescending(p => p.CastleCount)
                .ThenByDescending(p => p.CastlePower)
                .ThenByDescending(p => p.GameUserInfo.Supply)
                .ThenByDescending(p => p.GameUserInfo.Power)
                .ThenBy(p => p.GameUserInfo.ThroneInfluence).Select(p => p.GameUser1).ToList();

            //StringBuilder wallPostSB = new StringBuilder();
            //wallPostSB.Append(string.Format("'{0}' игра завершена.\n\n", this.Name));
            ...
            for (int i = 0; i < users.Count; i++)
            {
                if (users[i].Login != null)
                {
                    ...
                    //wallPostSB.Append(string.Format("[id{0}|{1}]\n", users[i].Login, this.GameHost.GamePortalServer.GetProfile(users[i].Login).ApiUser.Api["FIO"]));
                }
                ...
                //wallPostSB.Append(newStep.Message.Last().Value).Append('\n').Append('\n');
            }
            ...
            var response = ExtHttp.Post($"http://{GameHost.Config.Settings.ServerAddress}/ChatService/AddChat",
                JsonConvert.SerializeObject(new { roomId = this.Id, creator = "Вестерос", message = "dynamic_gameOver" }));
            //GameHost.AddChatFunc(this.Id, "Вестерос", "dynamic_gameOver");
